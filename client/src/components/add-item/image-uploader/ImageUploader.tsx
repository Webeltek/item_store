...
const GetUploadApiQuery = `
  query getImageUpload($asset: String!) {
    imageUploadUrl: url(routeId: "image-upload", assetType: $asset)
    imageDeleteUrl: url(routeId: "image-delete", assetType: $asset)
  }
`;
...
export function ImageUploader({
      ...
      if (data && data.imageDeleteUrl && image.url) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              json.error?.message ||
                `Failed to delete image. Status: ${response.status}`
            );
          }
  ...
  const [result] = useQuery({
    ...
    variables : { asset: 'images' }
  });
  const { data, fetching, error } = result;
  // {"data":{"imageUploadUrl":"http://localhost:3000/api/images/"}}
